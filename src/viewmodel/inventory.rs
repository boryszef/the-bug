...
/// Storage and the equipment combined into one list, sorted by [`Item`] order —
/// what craft/experiment/disassemble now draw on (storage first, the equipment
/// for any remainder — see `docs/equipment-and-storage.md`), so this is what
...
pub fn combined_sorted(player: &Player) -> Vec<(Item, u32)> {
    ...
    for (&item, &quantity) in &player.equipment {
...
mod tests {
    ...
    #[test]
    fn combined_sorted_merges_storage_and_equipment_quantities_for_the_same_item() {
        let mut player = Player::default();
        player.inventory.insert(Branch, 3);
        player.equipment.insert(Branch, 2);
        player.equipment.insert(Stone, 1);
