...
mod tests {
    ...
    #[test]
    fn fresh_game_has_both_quests_available_and_nothing_else() {
        ...
        assert_eq!(available, [QuestID::CraftArrows, QuestID::ExploreRuins]);
    }
    ...
    fn accepted_quest_becomes_active_and_leaves_the_available_list() {
        ...
        assert_eq!(active.quest.id, QuestID::CraftArrows);
        assert_eq!(active.progress, 0);
        assert_eq!(overview.available.len(), 1);
        assert_eq!(overview.available[0].id, QuestID::ExploreRuins);
    }
    ...
    fn completed_quest_appears_in_completed_and_clears_active() {
        ...
        assert_eq!(overview.completed[0].id, QuestID::CraftArrows);
    }
}
