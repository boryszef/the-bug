...
and the material-selection rules of the experiment overlay. Player-facing text
also depends on Rust's `{:?}` (Debug) formatting of `Material` / `TerrainType`.
...
state. `game` gains human-readable `Display` for `Material` and a name/symbol
...
- `Material`: `impl Display` (`StoneAxe` → `"Stone Axe"`), derive `Ord`
...
- Game event strings use `{}` (Display) instead of `{:?}` for `Material` /
  `TerrainType`.
...
- `src/viewmodel/inventory.rs` — `sorted(&Player) -> Vec<(Material, u32)>`
...
- `src/viewmodel/selection.rs` — `MaterialSelection`: the quantity/cap/available
  rules extracted from `ui::experiment::Experiment`
- `ui::experiment::Experiment` keeps cursor/focus/`handle_key`/`render` and
  delegates the material rules to `MaterialSelection`.
