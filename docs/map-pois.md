...
Cave and Ruins used to be `TerrainType` variants: a cave tile *was* cave
terrain, so it couldn't also be forest, and a search there turned up only cave
items. That coupled one idea (what does this ground look like) to another (is
there something notable here) and made "a cave in the woods" unrepresentable.

Now a `MapTile` carries a terrain **and** an `Option<Poi>` — a point of
interest sitting on top. `Poi` is `Cave` and `Ruins` today; `Village` moves
from `TerrainType` to `Poi` next (it's a POI too, only special in that it's
always dead-centre), and `Bridge` comes later. A tile has at most one POI:
that's just the `Option`.
...
- **Search yield.** A tile offers its terrain's items *and* its POI's, each
  ...
  (`Village`, once it's a POI, has no items of its own.)
...
`mapgen` produces terrain only — it no longer has a scatter step
(`docs/mapgen.md`). `Map::new` then lays the POI grid with `scatter_pois`:
`Cave` and `Ruins` on cells picked uniformly at random (never the centre) at
`CAVE_FRACTION` / `RUINS_FRACTION` of the tile count — the same densities the
scatter terrain used, still scaling with map size. (When `Village` becomes a
POI it goes in here too, pinned to the centre.)
...
**Clean break:** the old terrain codes `C` / `R` are gone. A save whose
...
- `Village` moving from `TerrainType` to `Poi` (and `mapgen` then covering
  every cell, no centre hole).
